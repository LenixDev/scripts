...
AddEventHandler("CheckThread", function()
    ...
    if closestPlayer == -1 or closestDistance > 2.0 then
        ...
        if not headMask then
            if handsup then
                TriggerServerEvent("Rc2-headbag:getClosestPlayer", GetPlayerServerId(closestPlayer));
                headMask = true;
            end
        else
            TriggerServerEvent("Rc2-headbag:getClosestPlayer2", GetPlayerServerId(closestPlayer));
            headMask = false;
        end
    end
end)
