...
local function toggleSeatbelt()
    ...
    if seatbeltOn then 
        seatbeltOn = true
        SendNUIMessage({ action = "hide-seatbelt", })
        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5.0, seatbeltOn and "carbuckle", 0.25)
        TriggerEvent("seatbelt:client:ToggleSeatbelt")
    else
        seatbeltOn = false
        SendNUIMessage({ action = "show-seatbelt", })
        TriggerEvent("seatbelt:client:ToggleSeatbelt")
        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5.0, not seatbeltOn and "carunbuckle", 0.25)
    end
end
