...
CreateThread(function()
    ...
    SetBlipSprite(pizzajobBlip, 389)
...
function ClockInPed()
    ...
    if not DoesEntityExist(pizzaBoss) then
        ...
        TaskPlayAnim(pizzaBoss, "amb@world_human_clipboard@male@base", "base", 8.0, 1.0, -1, 01, 0, 0, 0, 0)

        exports['qb-target']:AddTargetEntity(pizzaBoss, { 
            options = {
                { 
                    type = "client",
                    event = "pizzajob:client:startJob",
                ...
                { 
                    type = "client",
                    event = "pizzajob:client:finishWork",
                    icon = "fa-solid fa-pizza-slice",
                    label = "Finish Work",
                    canInteract = function()
                        return Hired
...
RegisterNetEvent('pizzajob:client:startJob', function()
    if not Hired then
        PullOutVehicle()
    end
end)
...
function NextDelivery()
    if not activeOrder then
        ...
        exports['qb-target']:AddCircleZone("deliverZone", vector3(newDelivery.x, newDelivery.y, newDelivery.z), 1.3,{ name = "deliverZone", debugPoly = false, useZ=true, }, { options = { { type = "client", event = "pizzajob:client:deliverPizza", icon = "fa-solid fa-pizza-slice", label = "Deliver Pizza"}, }, distance = 1.5 })
        activeOrder = true
        ...
        if Hired then
            if DecorExistOn((veh), "pizza_job") then
                ...
                if DeliveriesCount > 0 then
                    TriggerServerEvent('pizzajob:server:Payment', DeliveriesCount)
                else
