lib.addCommand(_U('COMMAND__ADMINMENU'), {
    ...
    local src = source
    if IsPlayerAceAllowed(src, 'drugs') then
        ...
        if menuType == 'plants' then
            TriggerClientEvent('drugs:client:showMainAdminMenu', src, {menuType = 'plants'})
        elseif menuType == 'tables' then
            TriggerClientEvent('drugs:client:showMainAdminMenu', src, {menuType = 'tables'})
        else
            ShowNotification(src, _U('NOTIFICATION__ADMINMENU__USAGE'):format(_U('COMMAND__ADMINMENU')), "info")
        end
    else
        ...
        TriggerClientEvent('drugs:client:showAdminAlertBox', src, {userLicense = userLicense, username = username})
    end
end)
...
lib.addCommand(_U('COMMAND__GROUNDHASH'), {
    ...
    local src = source
    if IsPlayerAceAllowed(src, 'drugs') then
        TriggerClientEvent('drugs:client:showGroundHash', src)
    else
        -- get user license
        local userLicense = GetPlayerIdentifiers(src)[1]
        local username = GetPlayerName(src)
