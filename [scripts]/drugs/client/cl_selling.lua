...
RegisterNetEvent('drugs:client:checkSellOffer', function(entity)
	local copsAmount = lib.callback.await('drugs:server:getCopsAmount', false)
	...
	TriggerEvent('drugs:client:showSellMenu', {item = sellItemData.item, price = sellItemData.price, amount = sellAmount, entity = entity})
	SetTimeout(Config.SellSettings['sellTimeout']*1000, function()
		...
		if lib.getOpenContextMenu() ~= nil then
			...
			if currentMenu == 'drugs-sell-menu' then
...
RegisterNetEvent('drugs:client:salesInitiate', function(cad)
	AddSoldPed(cad.tped)
	if cad.type == 'close' then
		...
		TriggerServerEvent('drugs:server:initiatedrug', cad)
		SetPedAsNoLongerNeeded(cad.tped)
	end
end)
