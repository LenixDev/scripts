...
local function plantSeed(ped, plant, plantInfos, plantItem, coords, metadata)
    ...
    local plants = lib.callback.await('drugs:server:getPlants', false)
    ...
    if ShowProgressBar({
        ...
        TriggerServerEvent('drugs:server:createNewPlant', coords, plantItem, zone, metadata)
        ClearPedTasks(ped)
...
-- Events
RegisterNetEvent('drugs:client:useSeed', function(plantItem, metadata)
    ...
    local ownedPlants = lib.callback.await('drugs:server:getPlantByOwner', false)
...
RegisterNetEvent('drugs:client:harvestPlant', function(args)
    ...
    if ShowProgressBar({
        ...
        TriggerServerEvent('drugs:server:harvestPlant', plantData.id)
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    else
...
local giveWater = function(args)
    ...
    if ShowProgressBar({
        ...
        TriggerServerEvent('drugs:server:plantTakeCare', plantData.id, item)
        ClearPedTasks(ped)
...
RegisterNetEvent('drugs:client:useItem', function (args)
    local item = args.item
...
RegisterNetEvent('drugs:client:destroyPlant', function(args)
    ...
    if ShowProgressBar({
        ...
        TriggerServerEvent('drugs:server:destroyPlant', {plantId = plantData.id})
        ClearPedTasks(ped)
        RemoveAnimDict('amb@medic@standing@kneel@base')
        RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
    else
...
RegisterNetEvent('drugs:client:startPlantFire', function(coords)
    local ped = PlayerPedId()
