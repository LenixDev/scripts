...
---@param framework string | 'autodetect' | 'qb-core' | 'ox_core' | 'es_extended'
---@return table | nil
local function detectFramwork(framework)

    local function detectOxCore()
        if GetResourceState('ox_core') == 'started' then
            local oxcore = require '@ox_core/lib/init'
            if oxcore then
                it.core = 'ox_core'
                return oxcore
            end
