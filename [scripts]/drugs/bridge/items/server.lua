	...
	if it.core == "ox_core" then
		if Config.Debug then
            if Config.Inventory == 'ox_inventory' then
                print('has items are handled by ox_inventory')
            end
        end
    end
...
function it.giveItem(source, item, amount, metadata)
	...
	if it.core == "ox_core" then
		if Config.Debug then
            if Config.Inventory == 'ox_inventory' then
                print('give items are handled by ox_inventory')
            end
        end
    end
...
lib.callback.register('drugs:server:giveItem', function(source, item, amount, metadata)
...
function it.removeItem(source, item, amount, metadata)
	...
	if it.core == "ox_core" then
		if Config.Debug then
            if Config.Inventory == 'ox_inventory' then
                print('remove items are handled by ox_inventory')
            end
        end
    end
...
lib.callback.register('drugs:server:removeItem', function(source, item, amount, metadata)
...
function it.createUsableItem(item, cb)
    if ConsumableItems[item] then print('[lib] The item ' .. item .. ' is already registered as a consumable item. Skipping the registration of this item.') end
	...
	if it.core == "ox_core" then
		if Config.Debug then
            if Config.Inventory == 'ox_inventory' then
                print('consumables items are added in the ox_inventory\'s data/items.lua')
            end
        end
    end
...
function it.getItemCount(source, item, metadata)
	...
	if it.core == "ox_core" then
		if Config.Debug then
            if Config.Inventory == 'ox_inventory' then
                print('get item count are handled by ox_inventory')
            end
        end
    end
...
function it.getItemLabel(source, itemName)
    local itemLabel
    if it.inventory == 'ox' then
        itemLabel = lib.callback.await('lib:client:getItemLabel', source, itemName)
    end
	...
	if it.core == "ox_core" then
		if Config.Debug then
            if Config.Inventory == 'ox_inventory' then
                print('get items label are handled by ox_inventory')
            end
        end
    end
...
lib.callback.register('lib:server:getItemLabel', function(source, itemName)
    local itemLabel = it.getItemLabel(source, itemName)
    return itemLabel
end)

lib.callback.register('lib:hasItem', function(source, item, amount)
    local hasItem = it.hasItem(source, item, amount)
    return hasItem
end)

lib.callback.register('lib:getItemCount', function(source, item)
    local itemCount = it.getItemCount(source, item)
    return itemCount
end)

RegisterNetEvent('lib:toggleItem', function(toggle, name, amount, metadata)
    local source = source
