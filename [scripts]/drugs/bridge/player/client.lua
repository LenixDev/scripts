...
function it.getCitizenId()
    local citizenId = lib.callback.await('lib:getCitizenId', false)
    return citizenId
end
...
function it.getPlayerJob()
    ...
    if it.core == 'ox_core' then
        job = {
            ---@TODO: Add the job data for the ox_core
        }
...
function it.getPlayerName()
    ...
    elseif it.core == 'esx' then
        return lib.callback.await('lib:getPlayerName', false)
    end
end

function it.getLicences()
    local licences = lib.callback.await('lib:getLicences', false)
    return licences
end

function it.getLicence(licenseType)
    local licence = lib.callback.await('lib:getLicence', false, licenseType)
    return licence
end