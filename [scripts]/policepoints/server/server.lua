...
QBCore.Commands.Add(Config.CheckPointsCommand, 'Check your police points', {}, false, function(source, args)
...
QBCore.Commands.Add(Config.Policemenucommand, 'Manage player police points', {}, false, function(source, args)
    local xPlayer = QBCore.Functions.GetPlayer(source)
    if xPlayer.PlayerData.job.name == 'police' and xPlayer.PlayerData.job.isboss then
        TriggerClientEvent('policepoints:client:openmenu', source)
    else
        TriggerClientEvent('QBCore:Notify', source, '  you cant do this',
            'error', 7000)
    end
end)
RegisterNetEvent('police:policemenu', function()
    local xPlayer = QBCore.Functions.GetPlayer(source)
    if xPlayer.PlayerData.job.name == 'police' and xPlayer.PlayerData.job.isboss then
        TriggerClientEvent('policepoints:client:openmenu', source)
    else
        TriggerClientEvent('QBCore:Notify', source, 'You can not do this',
            'error', 7000)
    end
end)
RegisterNetEvent('police:checkpoints', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == 'police' then
        TriggerClientEvent('QBCore:Notify', source, 'Police Points: ' .. Player.PlayerData.metadata['policepoints'],
            'success', 7000)
    else
        TriggerClientEvent('QBCore:Notify', source, 'You Cant use that',
            'error', 7000)
...
RegisterNetEvent('police:addpoints', function(amt)
    local xPlayer = QBCore.Functions.GetPlayer(source)
...
RegisterNetEvent('police:giveppoints', function(id, amt)
...
RegisterNetEvent('police:rmvppoints', function(id, amt)
    local p = QBCore.Functions.GetPlayerByCitizenId(id)
...
RegisterNetEvent('police:rmvallppp', function(id)
    local p = QBCore.Functions.GetPlayerByCitizenId(id)
    if p then
        p.Functions.SetMetaData('policepoints', 0)
    end
end)
QBCore.Functions.CreateCallback('playermanagement:server:getpoliceofficers', function(source, cb, args)
    local players = {}
