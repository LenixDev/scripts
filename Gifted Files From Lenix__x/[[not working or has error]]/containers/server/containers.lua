...
Citizen.CreateThread(function()
    exports['oxmysql']:execute("SELECT * FROM `containers`", function(result)
        if result[1] then 
            ...
            Wait(1000)
            TriggerClientEvent('containers:client:Container', -1, Config.Containers)
        end
    end)
end)

QBCore.Functions.CreateCallback('containers:server:GetContainerConfig', function(source, cb)
	cb(Config.Containers)
end)

RegisterNetEvent('containers:server:RentContainer', function(data)
    local src = source
...
RegisterNetEvent('containers:server:ContainerManageSrcond', function(data)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    local CanBuy = true
    if Player then 
...
RegisterNetEvent('containers:server:Key', function(data)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    local players = {}
    ...
    if #players > 0 then 
        TriggerClientEvent('containers:client:Key', src, data, players)
    else
        QBCore.Functions.Notify(src, 'No One Here', 'error', 7500)
    end
end)

RegisterNetEvent('containers:server:GiveKey', function(data)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player then 
        ...
        if Config.Containers.Locations[data.id].Owner == Player.PlayerData.citizenid then 
            if Target then 
                ...
                TriggerClientEvent('containers:client:Container', Target.PlayerData.source, Config.Containers)
            end
        else
...
RegisterNetEvent('containers:server:RemoveKeyList', function(data)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    local players = {}
    if Player then 
        ...
        if #players > 0 then 
            TriggerClientEvent('containers:client:RemoveKeyList', src, data, players)
        else
...
RegisterNetEvent('containers:server:Transfer', function(data)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    local players = {}
    local MyPed = GetPlayerPed(src)
    ...
    if #players > 0 then 
        TriggerClientEvent('containers:client:Transfer', src, data, players)
    else
        QBCore.Functions.Notify(src, 'No One Here', 'error', 7500)
    end
end)

RegisterNetEvent('containers:server:TransferConfirm', function(data)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayerByCitizenId(data.cid)
    if Target then 
        ...
        TriggerClientEvent('containers:client:TransferConfirm', Target.PlayerData.source, data)
    end
end)

RegisterNetEvent('containers:server:TransferAccept', function(data)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    if Player then 
        if not data then return end
...
RegisterNetEvent('containers:server:Cancel', function(data)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    local players = {}
    if Player then 
...
RegisterNetEvent('containers:server:RemoveKey', function(data)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    if Player then 
        if not data then return end
