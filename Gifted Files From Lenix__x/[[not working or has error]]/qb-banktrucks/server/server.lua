...
QBCore.Functions.CreateCallback('qb-banktrucks:server:hasItem', function(source, cb)
	...
	local hasItem = Player.Functions.GetItemByName("radioscanner")
    if hasItem then
        cb(true)
    else
        cb(false)
...
RegisterNetEvent('qb-banktrucks:server:giveitem', function ()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.AddItem("pinger", 1)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["pinger"], "add")
end)
...
QBCore.Functions.CreateUseableItem("pinger", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
