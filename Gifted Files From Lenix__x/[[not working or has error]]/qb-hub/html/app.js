...
$(document).ready(function () {
    $("body").on("keyup", function (key) {
        if (key.which === 113 || key.which == 27 || key.which == 90) {
            ...
            CallSignMenu = false;
            $('#main-title').html(`<div class="title-text">${PlayerJob === 'police' ? `Police List` : PlayerJob + ' List'}</div>`);
        }
    });
});
...
AddButtons = function (i) {
    $(document).on("mouseenter", `#Officer-${i['CitizenId']}`, function (e) {
        ...
        $(this).find('.name').css('color', 'rgb(0, 255, 206)')
    });
...
$(document).on("click", ".button", function (e) {
    ...
    switch (id) {
        case 'Confirm':
            ...
            CallSignMenu = false;
            $('.title').html(`<div class="title-text">${PlayerJob === 'police' ? `Police List` : PlayerJob + ' List'}</div>`);
            break;
        case 'Cancel':
            CallSignMenu = false;
            $('#main-title').html(`<div class="title-text">${oldTitle/*PlayerJob === 'police' ? `Police List` : PlayerJob + ' List'*/}</div>`);
            oldTitle = '';
            break;
    }
});
