...
function STR.execute(src, name, ...)
    local callID, solved = CallIdentifier, false
    CallIdentifier = CallIdentifier + 1

    Promises[callID] = promise:new()

    TriggerClientEvent("striez:rpc:request", src, Resource, name, callID, ParamPacker(...), true)
...
AddEventHandler("striez:rpc:request", function(origin, name, callID, params, packaged)
    local src = source
    ...
    TriggerClientEvent("striez:rpc:response", src, origin, callID, response)
end)
