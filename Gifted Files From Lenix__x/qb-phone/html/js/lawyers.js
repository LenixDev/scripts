SetupLawyers = function(data) {
    ...
    if (data.length > 0) {
        ...
        if (lawyers.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">لا يوجد محامون متاحون.</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (realestate.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">لا يوجد وكلاء عقاريين متاحين.</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (mechanic.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">لا يوجد ميكانيكي المتاحة.</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (taxi.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">لا توجد سيارات أجرة متاحة.</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (police.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">لا توجد شرطة متاحة.</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (ambulance.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">لا يوجد اسعاف المتاحة.</div></div>'
            $(".lawyers-list").append(element);
        }
    } else {
        $(".lawyers-list").append('<h1 style="font-size:1.641025641025641vh; padding:1.0256410256410255vh; color:#fff; margin-top:0; border-top-left-radius: .5vh; border-top-right-radius: .5vh; width:100%; display:block; background-color: rgb(42, 137, 214);">Lawyers (' + lawyers.length + ')</h1>');
        ...
        var element = '<div class="lawyer-list"><div class="no-lawyers">لا يوجد عساكر متاحون.</div></div>'
        $(".lawyers-list").append(element);
        
        $(".lawyers-list").append('<br><h1 style="font-size:1.641025641025641vh; padding:1.0256410256410255vh; color:#fff; margin-top:0; width:100%; display:block; background-color: rgb(255, 0, 0);">Ambulance (' + ambulance.length + ')</h1>');

        var element = '<div class="lawyer-list"><div class="no-lawyers">لا يوجد أفراد إسعاف متاحين.</div></div>'
        $(".lawyers-list").append(element);
    }
}
                ...
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (QB.Phone.Data.AnonymousCall) {
                            QB.Phone.Notifications.Add("fas fa-phone", "Phone", "! لقد بدأت مكالمة مجهولة");
                        }
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Phone", "! أنت متصل بالفعل بمكالمة");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Phone", "! هذا الشخص موجود بالفعل في مكالمة");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Phone", "! هاذا الشخص غير متوفر حاليا");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Phone", "! لا يمكنك الاتصال برقمك الخاص");
        }
    });
});
