...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "MDT", "! تم نسخ رقم المنزل", "#badc58", 1750);
    ...
    if (SearchName !== "") {
        $.post('https://qb-phone/FetchSearchResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                QB.Phone.Notifications.Add("politie", "MDT", "! لا توجد نتائج بحث");
                $(".person-search-results").html("");
            }
        });
    } else {
...
$(document).on('click', '.scan-search-vehicle', function(e){
    ...
    $.post('https://qb-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            ...
            QB.Phone.Notifications.Add("politie", "MDT", "! لا احد في الجوار");
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData != undefined){
        if (alertData.coords != undefined && alertData.coords != null) {
            ...
            QB.Phone.Notifications.Add("politie", "MDT", "! لا يحتوي هذا البلاغا على موقع");
        }
    }else {
        QB.Phone.Notifications.Add("politie", "MDT", "! لا توجد بلاغات متاحة.");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    $(".meos-alerts").html("");
    $(".meos-recent-alerts").html('<div class="meos-recent-alert"> <span class="meos-recent-alert-title">! ليس لديك اي بلاغات</span></div>');
    QB.Phone.Notifications.Add("politie", "MDT", "! تم حذف كل البلاغات");
});
