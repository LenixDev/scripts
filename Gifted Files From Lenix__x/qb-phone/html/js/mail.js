...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(QB.Phone.Data.PlayerData.charinfo.firstname+"."+QB.Phone.Data.PlayerData.charinfo.lastname+"@revival.com");
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        QB.Phone.Notifications.Add("fas fa-ad", "Advertisement", "! لا يمكنك نشر اعلان فارغ", "#ff8f1a", 2000);
    }
});
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    ...
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">لا توجد إعلانات حتى الآن!</span></div>';
        $(".advert-list").append(element);
    }
}
...
$(document).on('click','#adv-delete',function(e){
    ...
    $.post('https://qb-phone/DeleteAdvert', function(){
        setTimeout(function(){
            QB.Phone.Notifications.Add("fas fa-ad", "Advertisement", "تم حذف الإعلان", "#ff8f1a", 2000);
        },400)
    });
})
