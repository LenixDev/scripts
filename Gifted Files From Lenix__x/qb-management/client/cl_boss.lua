        ...
        if PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance' then 
            for _, v in pairs(cb) do
                ...
                EmployeesMenu[#EmployeesMenu + 1] = {
                    ...
                    txt = "Grade : "..v.grade.name.."<br>Duty hours : "..v.dutyhours.."<br>Dispatch points : "..v.dispatchpoints.."<br>Dispatch : "..isDispatch.."<br>Air unit : "..Wing.."",
                    params = {
...
RegisterNetEvent('qb-bossmenu:client:ManageEmployee', function(data)
    ...
    if PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance' then 
        ...
        local secMenu = {
            ["Grades"] = {
                lable = "Manage grade [Raise - Lower]",
                action = 1,
            },
            ["Duty"] = {
                lable = "Manage duty hours [Give - Remove]",
                action = 2,
            },
            ["Air unit"] = {
                lable = "Manage air unit [Give - Remove]",
                action = 3,
            },
            ["Dispatch"] = {
                lable = "Manage dispatch [Remove]",
                action = 4,
            },
        }
        --QBCore.Shared.Jobs[data.work.name].grades
        for k, v in pairs(secMenu) do
            EmployeeMenu[#EmployeeMenu + 1] = {
                header = k,
                txt = v.lable,
                params = {
                    isServer = false,
                    event = "qb-bossmenu:client:ActionMenu",
                    args = {
...
RegisterNetEvent('qb-bossmenu:client:ActionMenu', function(data)
    local EmployeeMenu = {
        {
            header = "Manage " .. data.player.name .. " - " .. string.upper(PlayerJob.label),
            isMenuHeader = true,
    ...
    elseif data.action == 2 then 
        EmployeeMenu = {
            ...
            {
                header = "Give",
                txt = "Give this person duty hours",
                params = {
                    isServer = false,
                    event = "qb-bossmenu:client:points",
                    args = {
                        cid = data.player.empSource,
                        player = data.player,
                        action = 1
            ...
            {
                header = "Remove",
                txt = "Remove duty hours this person",
                params = {
                    isServer = false,
                    event = "qb-bossmenu:client:points",
                    args = {
    ...
    elseif data.action == 3 then 
        EmployeeMenu = {
            ...
            {
                header = "Give",
                txt = "Give this person air unit",
                params = {
                    isServer = true,
                    event = "qb-bossmenu:server:wing",
                    args = {
                        cid = data.player.empSource,
                        action = 1
                    }
            ...
            {
                header = "Remove",
                txt = "Remove air unit from this person",
                params = {
                    isServer = true,
                    event = "qb-bossmenu:server:wing",
                    args = {
    ...
    elseif data.action == 4 then 
        TriggerServerEvent("qb-bossmenu:server:RemoveDispatch", data.cid)
    end
    EmployeeMenu[#EmployeeMenu + 1] = {
        header = "< Return",
        params = {
...
RegisterNetEvent('qb-bossmenu:client:points', function(data)
    local EmployeeMenu = {
        {
            header = "Manage " .. data.player.name .. " - " .. string.upper(PlayerJob.label),
            isMenuHeader = true,
        },
        {
            ...
            params = {
                isServer = true,
                event = "qb-bossmenu:server:addorRemovepoints",
                args = {
                    cid = data.cid,
                    action = data.action,
                    amount = 1
...
RegisterNetEvent('qb-bossmenu:client:SocetyDeposit', function(money)
    ...
    if deposit then
        ...
        TriggerServerEvent("qb-log:server:CreateLog", "depositbattal", "Deposit Balance :  ", "green", "   Added Money : $" .. deposit.amount .. "   New Balance: $ " ..money.. "")
    end
end)
...
RegisterNetEvent('qb-bossmenu:client:SocetyWithDraw', function(money)
    ...
    if withdraw then
        ...
        TriggerServerEvent("qb-log:server:CreateLog", "withdrawbattal", "Withdraw Balance :  ", "red", "  Remove Money : $" .. withdraw.amount .. "   New Balance: $ :" ..money.. "")
    end
end)
