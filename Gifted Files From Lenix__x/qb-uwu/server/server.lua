...
AddEventHandler('qb-uwu:server:pay:receipt', function(Price, Note, Id)
    ...
    if Player.Functions.RemoveMoney('bank', Price, 'cat-cafe') then
        if Config.ActivePayments[tonumber(Id)] ~= nil then
            ...
            TriggerClientEvent('qb-uwu:client:sync:register', -1, Config.ActivePayments)
			TriggerEvent('qb-bossmenu:server:addAccountMoney', "catcafe", Price)
		else
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if Player.PlayerData.job.name == "catcafe" and Player.PlayerData.job.onduty then
                Player.Functions.AddItem('burger_ticket', 1, false, {["quality"] = nil})
...
AddEventHandler('qb-uwu:Tickets:Sell', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName("burger_ticket") ~= nil then
		tickets = Player.Functions.GetItemByName("burger_ticket").amount
		pay = (tickets * Config.PayPerTicket)
		Player.Functions.RemoveItem('burger_ticket', tickets)
		Player.Functions.AddMoney('bank', pay, 'uwucafe-payment')
		TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['burger_ticket'], "remove", tickets)
