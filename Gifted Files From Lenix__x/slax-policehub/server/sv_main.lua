...
RegisterNetEvent('slax-policehub:server:addOfficer', function()
    local src = source
    ...
    Wait(250)
    TriggerClientEvent('slax-policehub:clint:update', -1, Officer)
end)

RegisterNetEvent('slax-policehub:server:deleteofiro', function()
    local src = source
...
RegisterNetEvent('slax-policehub:server:UpdateHub', function(type)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local Officers = ''
    local count = 0
    ...
    if type and type == 'all' then
        TriggerClientEvent('slax-policehub:clint:update', -1, { type = 'update', officers = Officers, count = count })
    else
        TriggerClientEvent('slax-policehub:clint:update', src, { type = 'update', officers = Officers, count = count })
    end
end)

RegisterNetEvent('slax-policehub:server:dispatch', function(duty)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerEvent('slax-policehub:server:UpdateHub', 'all')
end)

RegisterNetEvent('slax-policehub:server:duty', function(duty)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('slax-policehub:server:callsign', function(newSign)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('slax-policehub:server:GetCoords', function(source, cb, citizenid)
