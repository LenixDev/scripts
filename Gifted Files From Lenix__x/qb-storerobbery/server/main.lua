...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function(safe)
    ...
    if luck <= 10 then
        Player.Functions.AddItem('moneyroll1', math.random(3, 7))
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['moneyroll1'], 'add')
        if luck == odd then
            Wait(500)
            Player.Functions.AddItem('glaptop', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['glaptop'], 'add')
        end
    end
end)
