...
RegisterServerEvent("fivem-appearance:server:chargeCustomer", function(shopType)
    ...
    if Player.Functions.RemoveMoney('cash', money) then
        TriggerClientEvent("QBCore:Notify", src, "+ $" .. money .. " " .. shopType .. "!", "success")
    else
        TriggerClientEvent("QBCore:Notify", src, "لم يكن لديك ما يكفي من المال", "error")
    end
end)
    ...
    if model and components and props then
        MySQL.Async.insert(
            ...
            {Player.PlayerData.citizenid, name, model, json.encode(components), json.encode(props)}, function(id)
                ...
                TriggerClientEvent('QBCore:Notify', src, '' .. name .. ' تم الحفظ تحت اسم', 'success')
            end)
    end
end)
...
if Config.EnablePedMenu then
    QBCore.Commands.Add('pedmenu', 'قائمة الشكل', {{
        name = 'id',
        help = 'الايدي'
...
QBCore.Commands.Add('RC', 'إعادة تحميل شخصيتك', {}, false, function(source, _)
