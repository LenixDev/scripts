...
function takeHostage()
	...
	if closestPlayer ~= nil and closestPlayer ~= -1 then
		...
		for i=1, #hostageAllowedWeapons do
			if HasPedGotWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i]), false) then
				if GetAmmoInPedWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i])) > 0 then
					...
					QBCore.Functions.Notify("ليس لديك ذخيرة")
					return			
				end	
			else
				NoWeapon = true	
		...
		if NoWeapon then
			QBCore.Functions.Notify("لا تستطيع اخذ رهينة بهذا السلاح")
			return
		end
		...
		QBCore.Functions.Notify("لا يوجد احد")
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"~g~[G] Release ~w~, ~r~[H] Kill ")
