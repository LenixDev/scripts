...
Citizen.CreateThread(function() 
	...
	TriggerServerEvent("qb-carmusic:GetDate")
end)
...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "play" then
		if xSound:soundExists(nameid) then
			if xSound:isPaused(nameid) then
				TriggerServerEvent("qb-carmusic:ChangeState", true, nameid)
				local esperar = 0
				while nuiaberto do
					Wait(1000)
	...
	elseif data.action == "pause" then
		if xSound:soundExists(nameid) then
			if xSound:isPlaying(nameid) then
				TriggerServerEvent("qb-carmusic:ChangeState", false, nameid)
			end
		end
	...
	elseif data.action == "loop" then
		if xSound:soundExists(nameid) then
			...
			TriggerServerEvent("qb-carmusic:ChangeLoop",nameid,datasoundinfo.loop)
		else
			datasoundinfo.loop = not datasoundinfo.loop
		end
		if type(datasoundinfo.loop) ~= "table" then
			local loop = (''.. firstToUpper(tostring(datasoundinfo.loop))..' <b>التكرار</b>')
			SendNUIMessage({
				action = "changetextl",
				text = loop,
			})
	...
	elseif data.action == "forward" then
		if xSound:soundExists(nameid) then
			TriggerServerEvent("qb-carmusic:ChangePosition", 10, nameid)
		end
...
function ApplySound(quanti,plate)
	...
	if vadi <= 1.01 and vadi >= -0.001 and exis then
		...
		local volume = (("%".. math.floor((vadi*100) - 0.1+1) ..' <b>الصوت</b>'))
		SendNUIMessage({
            action = "changetextv",
            text = volume,
        })
		TriggerServerEvent("qb-carmusic:ChangeVolume", quanti, plate)
	end
end
...
function SetUrl(url,nid)
	...
	if url then
		...
		if encontrad then
			...
			TriggerServerEvent("qb-carmusic:ModifyURL",vehdata)
		else
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				local veh = GetVehiclePedIsIn(PlayerPedId(),false)
				...
				vehdata.loop = datasoundinfo.loop
				TriggerServerEvent("qb-carmusic:AddVehicle",vehdata)
			end
		end
	else
...
if not Config.ItemInVehicle then
	RegisterKeyMapping('carradio', 'qb-carmusic', 'keyboard', 'N')
...
if Config.ItemInVehicle then
	RegisterNetEvent("qb-carmusic:ShowNui")
	AddEventHandler("qb-carmusic:ShowNui", function()
		show()
	end)
end
...
function show(nomecenas)
    ...
    if shown and nome then
		...
		local volume = ("%".. math.floor((datasoundinfo.volume*100) - 0.1+1).." <b>الصوت</b>")
		if type(datasoundinfo.loop) ~= "table" then
			local loop = (''.. firstToUpper(tostring(datasoundinfo.loop)).." <b>التكرار</b>")
			SendNUIMessage({
				action = "changetextl",
    ...
    elseif nuiaberto then
		...
		QBCore.Functions.Notify("لا يمكنك فعل هذا الآن")
	end
end
...
AddEventHandler("qb-carmusic:AddVehicle", function(data)
...
AddEventHandler("qb-carmusic:ChangeState", function(tipo, nome)
...
AddEventHandler("qb-carmusic:ChangeLoop", function(tipo, nome)
	if xSound:soundExists(nome) then
...
AddEventHandler("qb-carmusic:ChangeVolume", function(som, range, nome)
	local carroe
	local crds
    for i = 1, #Zones do
        local v = Zones[i]
...
RegisterNetEvent("qb-carmusic:SendData")
AddEventHandler("qb-carmusic:SendData", function(data)
    Zones = data
    for i = 1, #Zones do
		local v = Zones[i]
		if v.isplaying then
		...
		for i = 1, #Config.Zones do
			...
			if myjob.name == v.job then
				...
				if distance <= 10 then
					...
					if distance <= 3 then
						dormir = 5
						DrawText3D(v.changemusicblip.x, v.changemusicblip.y, v.changemusicblip.z, "~r~w~w~ - ﺔﻴﻨﻏﻻﺍ ﺮﻴﻐﺗ")
			...
			if jobnil then
				if v.job == nil then
					...
					if distance <= 10 then
						...
						if distance <= 3 then
							dormir = 5
							DrawText3D(v.changemusicblip.x, v.changemusicblip.y, v.changemusicblip.z, "~g~E~w~ - ﺔﻴﻨﻏﻻﺍ ﺮﻴﻐﺗ")
							if IsControlJustReleased(0, 38) then
								nomidaberto = v.name
								show(v.name)
								Wait(1000)
