...
function AddPropToPlayerAndAnim(prop1, bone, off1, off2, off3, rot1, rot2, rot3)
    ...
    TaskPlayAnim(Player, "amb@world_human_mobile_film_shocking@female@base", "base", 2.0, 1.0, -1, 49, 0, 0, 0, 0)
end
...
RegisterNetEvent('QBCore:Player:SetPlayerData', function(val)
    if holdingMega then
        ...
        if not _HasMegaphone then
            ...
            exports["qb-voice"]:clearProximityOverride()
        end
    end
end)
...
RegisterNetEvent("megaphone:Toggle", function()
    if not holdingMega then
        ...
        exports["qb-voice"]:overrideProximityRange(50.0, true)
    else
        holdingMega = false
        ClearPedTasksImmediately(PlayerPedId())
        DeleteEntity(prop)
